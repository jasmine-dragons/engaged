...
class WebSocketHandler {
  static async setupWebSocket(serverUrl) {
    ...
    signalingSocket.onerror = (error) => {
      UIController.addSignalingLog("Signaling Socket Error", {
        error: error.message,
      });
    };
    ...
    signalingSocket.onmessage = (event) => {
      try {
        ...
        UIController.addSignalingLog("Error Processing Message", {
          error: error.message,
        });
      }
    };
  ...
  static handleError = (error) => {
    UIController.addSystemLog("Media Socket", "Connection error", {
      error: error.message,
    });
    MediaHandler.stopRecording();
    UIController.handleStop();
  ...
  static handleVideoData = async (event) => {
    if (
      event.data.size > 0 &&
      RTMSState.mediaSocket?.readyState === WebSocket.OPEN &&
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        if (RTMSState.mediaSocket?.readyState === WebSocket.OPEN) {
          UIController.addSystemLog("Media", `${type} sent`, {
            timestamp: Date.now(),
          });
          RTMSState.mediaSocket.send(
            JSON.stringify({
              msg_type: type,
              content: {
                user_id: 0,
      ...
      UIController.addSystemLog("Media", "Error sending media data", {
        error: error.message,
      });
    }
  }
        ...
        audioNode.port.onmessage = (event) => {
          ...
          if (
            ...
            const base64Data = btoa(
              String.fromCharCode.apply(null, new Uint8Array(pcmArray.buffer))
            );
  ...
  static sendSessionStateUpdate(state, stopReason) {
    if (
      !RTMSState.mediaSocket ||
      RTMSState.mediaSocket.readyState !== WebSocket.OPEN
    )
      return;

    UIController.addSystemLog("Session", `State updated to ${state}`, {
      stopReason,
    });
    RTMSState.mediaSocket.send(
      JSON.stringify({
