class UIController {
  ...
  static attachEventListeners() {
    document
      ...
      .addEventListener("click", APIHandler.validateWebhook);
    ...
    document.getElementById("startRtmsBtn").addEventListener("click", () => {
      ...
      if (window.lastWebhookPayload) {
        ...
        this.showError(
          "No previous meeting found. Please start a new meeting first."
        );
      }
    });
    document
      .getElementById("endBtn")
      .addEventListener("click", () => this.handleEnd());
  ...
  static handlePause() {
    ...
    try {
      ...
      WebSocketHandler.sendSessionStateUpdate(
        CONFIG.STATES.PAUSED,
        "ACTION_BY_USER"
      );
  ...
  static handleStateUpdate(message) {
    ...
    if (
      message.msg_type === "SESSION_STATE_UPDATE" ||
      message.msg_type === "UI_STATE_UPDATE"
    ) {
