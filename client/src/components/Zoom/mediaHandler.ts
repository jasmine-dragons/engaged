...
export class MediaHandler {
  ...
  static async setupVideoDisplay(mediaVideo: HTMLVideoElement) {
    ...
    await mediaVideo
      .play()
      .catch((e) => console.error("Error playing media video:", e));
  ...
  static async setupMediaRecorders() {
    ...
    if (
      !RTMSState.videoRecorder ||
      RTMSState.videoRecorder.state === "inactive"
    ) {
  ...
  static setupRecorderEventHandlers() {
    ...
    RTMSState.audioRecorder.onerror = (e) =>
      logDebug(`Audio recorder error: ${e.name}`);
  }
